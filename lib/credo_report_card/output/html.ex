defmodule CredoReportCard.Output.Html do
  @moduledoc false
  ...
  def print_issues(issues) do
    issues
    |> Enum.reduce(Map.new(), &CredoReportCard.Formatter.categorize_module/2)
    |> write_html()
  end
  ...
  defp module_template(io, k, v) do
    {issue_count, raw_score} = CredoReportCard.Formatter.score_issues(k, v)
    mod_grade = CredoReportCard.Formatter.grade(raw_score)
    ...
    IO.binwrite(io, [
      ...
      CredoReportCard.Formatter.format_remediation_time(raw_score),
      "</td>",
      "</tr>\n"
    ])
